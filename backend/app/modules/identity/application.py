...
from backend.app.core.security import create_token, decode_token, token_digest, verify_password
...
class AuthenticationService:
    ...
    def refresh(self, refresh_token: str) -> tuple[User, str, str]:
        ...
        session = self.db.scalar(select(RefreshSession).where(RefreshSession.token_id == payload["jti"]))
        if not session or session.revoked_at or session.token_hash != token_digest(refresh_token):
    ...
    def logout(self, refresh_token: str) -> None:
        try:
            ...
            return
        session = self.db.scalar(select(RefreshSession).where(RefreshSession.token_id == payload.get("jti")))
    ...
    def _issue_tokens(self, user: User) -> tuple[str, str]:
        ...
        access, _, _ = create_token(str(user.id), "access", timedelta(minutes=settings.access_token_minutes))
