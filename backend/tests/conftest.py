...
def client(tmp_path):
    ...
    with TestingSession() as db:
        ...
        db.add_all(
            [
                User(email="admin@example.com", full_name="Admin", password_hash=hash_password("Password123!"), role=Role.ADMINISTRATOR, branch_id=branch.id),
                User(email="viewer@example.com", full_name="Viewer", password_hash=hash_password("Password123!"), role=Role.VIEWER, branch_id=branch.id),
            ]
        )
        db.commit()
